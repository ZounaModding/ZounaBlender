...
class ImportSkinZ(bpy.types.Operator, ImportHelper):
    ...
    gameVersion: EnumProperty(
        items=blender_props(), name="Imported File Version", default=0
    )
...
class ImportMeshZ(bpy.types.Operator, ImportHelper):
    bl_idname = "import_scene.meshz"
    bl_label = "Import Mesh_Z"
    bl_description = "Import Mesh_Z file"
    bl_options = {"PRESET"}

    filename_ext = ".Mesh_Z"
    filter_glob: StringProperty(default="*.Mesh_Z", options={"HIDDEN"})

    @staticmethod
    def blender_props() -> List[Tuple[str, str, str]]:
        return [
            ("Ratatouille", "Ratatouille", "Import Ratatouille Mesh_Z"),
            ("Wall-E", "Wall-E", "Import Wall-E Mesh_Z"),
        ]
    ...
    def execute(self, context):
        ...
        if result:
            self.report({"INFO"}, "Mesh_Z has been loaded")
            return {"FINISHED"}

        else:
            print("Failed to load Mesh_Z")
            #self.report({"ERROR"}, "Failed to load Mesh_Z")
            return {"CANCELLED"}
...
class ImportAnimZ(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if result:
            ...
            print("Failed to load Animation_Z")
...
def menu_func_meshz_import(self, context):
    self.layout.operator(ImportMeshZ.bl_idname, text="Mesh_Z (.Mesh_Z)")
...
def register():
    ...
    bpy.utils.register_class(ImportMeshZ)
    bpy.types.TOPBAR_MT_file_import.append(menu_func_meshz_import)
...
def unregister():
    ...
    bpy.utils.unregister_class(ImportMeshZ)
    bpy.types.TOPBAR_MT_file_import.remove(menu_func_meshz_import)

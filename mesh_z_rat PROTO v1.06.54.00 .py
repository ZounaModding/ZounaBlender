...
def readMesh_Z(f, subsectionMatIndices, rootpath, meshBoneCrc32s=None):
    ...
    uvShortCount = reader.read_int32()
    for i in range(uvShortCount):
        uvs.append(reader.read_int16())

    normalByteCount = reader.read_int32()
    ...
    displayListCount = reader.read_int32()
    for i in range(displayListCount):
        blender_verts = []

        bm = bmesh.new()
        uv_layer = bm.loops.layers.uv.new()
        bytes = reader.read_int32()
        print(str(bytes))
        reader.read_int32()
        ...
        newobject = bpy.data.objects.new(
            ("Mesh_" + name + "_DispList_" + str(i)), newmesh
        )
...
def loadOne(path):
    pathAll = "C:\\Users\\Sabe\\Downloads\\_Setups\\dolphin-2407-x64\\Games\\rat_proto\\files\\WORLD\\ext\\MB_RIVER.DGC.out\\resources"
    global rootpath
    rootpath = pathAll + "\\"
    loadAll(pathAll)
    # loadOne(pathOne)

...
def readAnimation_Z(f, rig, maxFrames):
    ...
    datasize = reader.read_int32()
    # print(str(datasize))
    linksize = reader.read_int32()
    # print(str(linksize))
    decompressedsize = reader.read_int32()
    # print(str(decompressedsize))
    compressedsize = reader.read_int32()
    # print(str(compressedsize))
    classcrc32 = reader.read_int32()
    ...
    rotframeCount = reader.read_int32()
    ...
    bezierRotframeCount = reader.read_int32()
    reader.read_bytes(bezierRotframeCount * 36)

    node_scaleframe_list = []

    reader.read_bytes(2)
    scaleframeCount = reader.read_int32()
    ...
    reader.read_bytes(2)
    transframeCount = reader.read_int32()
    ...
    reader.read_uint16()
    floatframeCount3 = reader.read_int32()
    ...
    reader.read_bytes(2)
    floatframeCount4 = reader.read_int32()
    reader.read_bytes(floatframeCount4 * 12)

    # AnimNodeModifier

    animNodeModifierCount = reader.read_int32()
    animNodeModifierValueList = []
    for i in range(0, animNodeModifierCount):
        animNodeModifierValueList.append(reader.read_int32())
        animNodeModifierValueList.append(reader.read_int32())
    ...
    for i in range(0, len(rig.pose.bones)):
        ...
        for tIndex in range(
            boneTransStartFrame, boneTransStartFrame + boneTransFrameCount
        ):
            # print(tIndex*10)
            ...
            poseSpaceTranslation = (
                editBone.matrix_local.inverted()
                @ editBone.parent.matrix_local
                @ Matrix.Translation([xVal, yVal, zVal])
            ).translation

            keyframeX = fCurveTransX.keyframe_points.insert(
                frame, poseSpaceTranslation.x
            )
            ...
            keyframeY = fCurveTransY.keyframe_points.insert(
                frame, poseSpaceTranslation.y
            )
            ...
            keyframeZ = fCurveTransZ.keyframe_points.insert(
                frame, poseSpaceTranslation.z
            )
        ...
        for rIndex in range(boneRotStartFrame, boneRotStartFrame + boneRotFrameCount):
            ...
            poseSpaceRot = (
                editBone.matrix_local.to_quaternion().inverted().to_matrix().to_4x4()
                @ editBone.parent.matrix_local
            ...
            if rIndex < boneRotStartFrame + boneRotFrameCount - 1:
                ...
                poseSpaceRotNext = (
                    editBone.matrix_local.to_quaternion()
                    .inverted()
                    .to_matrix()
                    .to_4x4()
                    @ editBone.parent.matrix_local
                    @ quatNext.to_matrix().to_4x4()
                ).to_quaternion()
                ...
                if (
                    (
                        (xInterp - poseSpaceRotInterp.x > 0.5)
                        or (xInterp - poseSpaceRotInterp.x < -0.5)
                    )
                    or (
                        (yInterp - poseSpaceRotInterp.y > 0.5)
                        or (yInterp - poseSpaceRotInterp.y < -0.5)
                    )
                    or (
                        (zInterp - poseSpaceRotInterp.z > 0.5)
                        or (zInterp - poseSpaceRotInterp.z < -0.5)
                    )
                    or (
                        (wInterp - poseSpaceRotInterp.w > 0.5)
                        or (wInterp - poseSpaceRotInterp.w < -0.5)
                    )
                ):
                    ...
                    print(
                        "    - quatLerpInterp: "
                        + str(Quaternion((wInterp, xInterp, yInterp, zInterp)))
                    )
                    ...
                    keyframeXfix = fCurveRotX.keyframe_points.insert(
                        frame + 1, poseSpaceRotInterp.x
                    )
                    keyframeXfix.interpolation = "LINEAR"

                    keyframeYfix = fCurveRotY.keyframe_points.insert(
                        frame + 1, poseSpaceRotInterp.y
                    )
                    keyframeYfix.interpolation = "LINEAR"

                    keyframeZfix = fCurveRotZ.keyframe_points.insert(
                        frame + 1, poseSpaceRotInterp.z
                    )
                    keyframeZfix.interpolation = "LINEAR"

                    keyframeWfix = fCurveRotW.keyframe_points.insert(
                        frame + 1, poseSpaceRotInterp.w
                    )
        ...
        for sIndex in range(
            boneScaleStartFrame, boneScaleStartFrame + boneScaleFrameCount
        ):
            ...
            poseSpaceScale = (
                editBone.matrix_local.inverted()
                @ editBone.parent.matrix_local
                @ Matrix.Diagonal(Vector([xVal, yVal, zVal])).to_4x4()
            ).to_scale()
    ...
    animMaterialModifierCount = reader.read_int32()
    reader.read_bytes(28 * animMaterialModifierCount)

    # AnimMeshModifier

    animMeshModifierCount = reader.read_int32()
    reader.read_bytes(12 * animMeshModifierCount)

    # AnimMorphModifier

    animMorphModifierCount = reader.read_int32()
...
def readSkel_Z(bs):
    ...
    for i in range(0, bone_count):
        ...
        bs.seek(1 * 2 * 4, 1)
        boneNames.append(str(bs.read_int32()))
        bones.append((Matrix(trs), parent_index))

    material_crc32_count = bs.read_int32()
    bs.seek(material_crc32_count * 1 * 4, 1)

    mesh_data_crc32_count = bs.read_int32()
    bs.seek(mesh_data_crc32_count * 1 * 4, 1)

    animation_node_names_arrays = bs.read_int32()
    for i in range(0, animation_node_names_arrays):
        crc32s = bs.read_int32()
        bs.seek(crc32s * 1 * 4, 1)

    some_names_crc32_count = bs.read_int32()
    bs.seek(some_names_crc32_count * 1 * 4, 1)

    sphere_col_bones0_count = bs.read_int32()
    bs.seek(sphere_col_bones0_count * 7 * 4, 1)

    sphere_col_bones1_count = bs.read_int32()
    bs.seek(sphere_col_bones1_count * 7 * 4, 1)

    box_col_bones_count = bs.read_int32()
    bs.seek(box_col_bones_count * 19 * 4, 1)


def execute(skelCrc32, skinCrc32, rootpath):
    bs = binary_reader.BinaryReader(
        open(rootpath + "\\" + (str(skelCrc32) + ".Skel_Z"), "rb").read()
    )
    ...
    if bpy.data.armatures.get("Skin_" + str(skinCrc32) + "_skeleton") != None:
        bpy.data.armatures.remove(
            bpy.data.armatures.get("Skin_" + str(skinCrc32) + "_skeleton")
        )
...
def readMesh_Z(f, subsectionMatIndices, rootpath, meshBoneCrc32s=None):
    ...
    reader = binary_reader.BinaryReader(f.read())
    reader.read_bytes(16)  # bff header
    datasize = reader.read_int32()
    linksize = reader.read_int32()
    decompressedsize = reader.read_int32()
    compressedsize = reader.read_int32()
    classcrc32 = reader.read_int32()
    namecrc32 = reader.read_int32()
    name = str(namecrc32)
    print("loading mesh " + name)
    reader.seek(linksize, whence=1)
    reader.read_int32()
    reader.read_int32()
    morpherRelatedCount = reader.read_int32()
    ...
    reader.read_int32()
    reader.read_int32()
    matCrc32Count = reader.read_int32()
    print("material count: " + str(matCrc32Count))
    matCrc32List = []
    matList = []
    addMatIndices = False
    ...
    for i in range(matCrc32Count):
        matCrc32List.append(str(reader.read_int32()))
        print("material_crc32[i]: " + str(i) + " " + str(matCrc32List[i]))
        if addMatIndices:
            subsectionMatIndices.append(i)
    rootpath += "\\"
    ...
    for i in range(shortvertexcount * 3):
        reader.read_uint16()
    unkUintCount = reader.read_int32()
    if unkUintCount != 0:
    ...
    for i in range(vertexGroupCount):
        ...
        reader.read_uint16()
        vertexCountInBuffer = reader.read_int32()
        print("vcount" + str(vertexCountInBuffer))
        faceBufferOffset = reader.read_int32()
        print("ibuffer off " + str(faceBufferOffset))
        faceCountInBuffer = reader.read_int32()
        ...
        for j in range(vertexCountInBuffer):
            ...
            weights = []
            if vertexSize == 60:
                if meshBoneCrc32s is None:
                    ShowMessageBox("Info: Skipped skinned mesh (needs Skin_Z)", "Alert", "ERROR")
                    print(f"Info: Skipped skinned mesh {name} (needs Skin_Z)")
                    return False
                ...
                for k in range(0, len(weightIndices)):
                    ...
                    if curBoneName == -1 or curBoneName == -2:
        ...
        for k in range(faceCountInBuffer):
            try:
                face = bm.faces.new(
                    [vertices_list[reader.read_uint16()] for x in range(3)]
                )
        ...
        newobject = bpy.data.objects.new(
            ("Mesh_" + name + "_VertGrp_" + str(i)), newmesh
        )
        ...
        newobject.data.materials.append(
            bpy.data.materials.get(matCrc32List[subsectionMatIndices[i]])
        )
...
def readSkin(f, path):
    ...
    classcrc32 = reader.read_int32()
    namecrc32 = reader.read_int32()
    linkCrc32 = reader.read_int32()
    linkCount = reader.read_int32()
    reader.seek(4 * linkCount, 1)
    skelCrc32 = reader.read_int32()
    if skelCrc32 != 0:
    ...
    if isClassId != 0:
        ...
        for i in range(reader.read_int32()):
            reader.seek(8, 1)
    matrixCacheCheck = reader.read_int32()
    skinSectionCount = reader.read_int32()
    ...
    for i in range(skinSectionCount):
        ...
        oldMaterial = -1
        curIndex = -1
        skinSubSectionCount = reader.read_int32()
        for j in range(skinSubSectionCount):
            curMaterial = reader.read_int32()
            if curMaterial != oldMaterial:
                curIndex += 1
            for x in range(7):
                bone = reader.read_int32()
                boneCrc32ForSection.append(bone)
            reader.seek(8, 1)
            morphPacketCount = reader.read_int32()
    ...
    if False and rig != None:
        ...
        for animFilePath in animFiles:
            ...
            animFile = open(
                animFilePath, "rb"
            )  # open(rootpath+'\\3658503735.Animation_Z', "rb")#
...
def loadOne(path):
    ...
    if path.endswith(".Skin_Z"):
        ...
        print("loading: " + path)
        print("-------------------------------------------------------------")
        return readSkin(f, path)
    elif path.endswith(".Mesh_Z"):
        f = open(path, "rb")
        print("loading: " + path)
        print("-------------------------------------------------------------")
        rootopatho = os.path.dirname(path)
        return readMesh_Z(f, [], rootopatho)

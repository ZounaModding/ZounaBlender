...
def execute(skelCrc32, skinCrc32, rootpath):
    bs = binary_reader.BinaryReader(
        open(rootpath + os.sep + (str(skelCrc32) + ".Skel_Z"), "rb").read()
    )
...
def readSkin(f, path):
    ...
    if skelCrc32 != 0:
        ...
        ShowMessageBox(
            "Unsupported Skin_Z!! Skeleton will NOT be loaded", "Alert", "ERROR"
        )
    ...
    for i in range(skinSectionCount):
        ...
        mf = open(meshName, "rb")
        readMesh_Z(
            mf, subSectionMaterialIndices, rootpath, meshBoneCrc32s if rig else []
        )
